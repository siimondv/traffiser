...
main(){
    ...
    // Vector to store the connections
    ...
    newConnection.connection_header = connection;
    connections.push_back(newConnection);
    std::string connection2 = "1---------------------------1";
    struct connection newConnection2;
    newConnection2.connection_header = connection2;
    ...
    keypad(stdscr, TRUE);
    nodelay(stdscr, TRUE);// Enable keypad mode for arrow keys
    ...
    while (true) {
        ...
        for (int i = startIdx; i < endIdx; ++i) {
            if(i == pressed_connection){
                pressed_connection = -1;
                selected = 0;
                startIdx = 0;
            }
            else{
                /*
             * by turning on A_REVERSE, we can highlight the selected word. We have to turn it off right after,
             * otherwise all the words will be highlighted.
             */
                if (i == selected) {
        ...
        int ch = getch(); // breaks when there are threads
/*
        ...
        // Your code goes here
        std::cout << "emntroooooooo" << std::endl;
